...
gd = generate_data(1000,1000,classification = TRUE, additive = FALSE)
data = gd[["data"]]
labels = gd[["labels"]]
...
for(M in c(1,5,10,50,100,200)){
  ...
  params_list[[paste0("m", M)]] =
    list(M = M,   								# number elementary FS runs
         ...
         popsize = 100,
         maxiter = 100,
         max_size = 3,								# number of features for Hamming prior / ensemble
...
res = feature_f1Score(res = res, true_indices = c(1,2,3))
